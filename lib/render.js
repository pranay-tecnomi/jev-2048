...
function renderFrame(state) {
  ...
  let rightBody
  if (status === 'error') {
    rightBody = [RED + BOLD + 'Request failed' + RESET, '', RED + String(errorMessage || '').slice(0, 40) + RESET]
  } else if (status === 'thinking') {
    rightBody = [ORANGE + 'Awaiting response' + RESET + GRAY + ' ...' + RESET]
  } else if (lastDecision) {
    rightBody = [
      GRAY + 'NEXT MOVE' + RESET,
      '',
      ...probRows(lastDecision.probabilities, lastDecision.choice),
      '',
      GRAY + 'EXECUTING   ' + RESET + ORANGE + BOLD + lastDecision.choice + RESET,
      GRAY + 'CONFIDENCE  ' + RESET + WHITE + (lastDecision.confidence != null ? Math.round(lastDecision.confidence * 100) + '%' : '-') + RESET,
      GRAY + 'LATENCY     ' + RESET + WHITE + Math.round(lastDecision.latencyMs) + 'ms' + RESET,
    ]
  } else {
    rightBody = [GRAY + 'Waiting for first decision...' + RESET]
  }
  ...
  if (status === 'won') {
    lines.push(row(GOLD + BOLD + 'REACHED 2048! Final score: ' + score + RESET))
  } else if (status === 'lost') {
    lines.push(row(RED + BOLD + 'GAME OVER. Final score: ' + score + RESET))
  } else {
    lines.push(row(GRAY + 'Ctrl+C quit' + RESET))
  }
...
module.exports = { renderFrame, clearScreen }
